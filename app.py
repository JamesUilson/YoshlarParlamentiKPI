...
def dashboard():
    ...
    c.execute("SELECT * FROM reports WHERE user_id = ? ORDER BY submitted_date DESC", (session['user_id'],))
    reports = c.fetchall()
    
    # Topshiriqlar
    c.execute("SELECT * FROM user_tasks WHERE (assigned_to = ? OR assigned_to = 'all') AND status != 'completed' ORDER BY deadline ASC LIMIT 5", 
              (session['user_id'],))
    tasks = c.fetchall()
    
    # Reyting
    c.execute("SELECT rating FROM users WHERE user_id = ?", (session['user_id'],))
...
def api_dashboard_stats():
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM reports WHERE user_id = ? AND status = 'rated'", (user_id,))
...
def api_download_report(report_id):
    ...
    try:
        ...
        c.execute('SELECT user_id, file_path FROM reports WHERE id = ?', (report_id,))
        report = c.fetchone()
...
def api_user_tasks():
    ...
    try:
        ...
        # Asosiy WHERE sharti
        where_conditions = ['(ut.assigned_to = ? OR ut.assigned_to = "all")']
        params = [user_id]
        
        # Status filtri
        if status_filter != 'all':
            where_conditions.append('ut.status = ?')
            params.append(status_filter)
        
        # Priority filtri
        if priority_filter != 'all':
            where_conditions.append('ut.priority = ?')
...
def api_user_rating_statistics():
    ...
    try:
        ...
        c.execute("SELECT district, rating FROM users WHERE user_id = ?", (user_id,))
...
def api_faq():
    faq_data = [
        {
            'question': 'Hisobot qanday topshiriladi?',
        ...
        {
            'question': 'Hisobotim necha kunda baholanadi?',
        ...
        {
            'question': 'Topshiriq qanday bajariladi?',
        ...
        {
            'question': 'Hisobotni yuklab olish mumkinmi?',
        ...
        {
            'question': 'Qanday qilib reytingimni oshirishim mumkin?',
                ...
                if column_name not in existing_columns:
                    ...
                    if 'DEFAULT' in column_type:
                        try:
                            ...
                            update_sql = f'UPDATE user_tasks SET {column_name} = ? WHERE {column_name} IS NULL'
...
def api_ratings_table():
    ...
    try:
        ...
        # Qidirish
        if search_term:
            where_clauses.append("(u.full_name LIKE ? OR u.user_id LIKE ?)")
...
def api_get_report_files(report_id):
    ...
    try:
        conn = sqlite3.connect(app.config['DATABASE'])
        c = conn.cursor()
        c.execute("SELECT file_path FROM reports WHERE id = ?", (report_id,))
        result = c.fetchone()
        conn.close()
...
def api_rating_stats_v2():
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM users WHERE role = 'user' AND last_login > ?", (thirty_days_ago,))
...
def api_rate_report_post(report_id):
    ...
    try:
        ...
        c.execute("UPDATE reports SET status = 'rated', admin_comment = ? WHERE id = ?", 
                 (comment, report_id))
...
#         # Qidirish
#         if search_term:
#             query += " AND (u.full_name LIKE ? OR u.user_id LIKE ?)"
#             params.append(f'%{search_term}%')
#             params.append(f'%{search_term}%')
...
def generate_test_ratings():
    ...
    try:
        conn = sqlite3.connect(app.config['DATABASE'])  # conn qo'shildi
...
def api_admin_delete_user(user_id):
    ...
    try:
        ...
        c.execute("DELETE FROM reports WHERE user_id=?", (user_id,))
        c.execute("DELETE FROM user_tasks WHERE assigned_to=?", (user_id,))
...
def api_delete_task(task_id):
    ...
    try:
        ...
        c.execute("SELECT * FROM user_tasks WHERE task_id = ?", (task_id,))
        task = c.fetchone()
        ...
        if session.get('role') == 'debugger':
            ...
            c.execute("SELECT assigned_by FROM user_tasks WHERE task_id = ?", (task_id,))
            ...
            c.execute("DELETE FROM user_tasks WHERE task_id = ? AND assigned_by = ?", 
...
def api_update_task(task_id):
    ...
    try:
        ...
        if 'rating_given' in data:
            update_fields.append("rating_given = ?")
        ...
        if session.get('role') == 'debugger':
            ...
            query = f"UPDATE user_tasks SET {', '.join(update_fields)} WHERE task_id = ?"
            ...
            query = f"UPDATE user_tasks SET {', '.join(update_fields)} WHERE task_id = ? AND assigned_by = ?"
    ...
    try:
        ...
        if deadline != 'all':
            ...
            if deadline == 'today':
                ...
                query += " AND t.deadline BETWEEN DATE(?) AND DATE(?, '+7 days')"
                params.append(today)
                params.append(today)
            elif deadline == 'month':
                query += " AND t.deadline BETWEEN DATE(?) AND DATE(?, '+30 days')"
                params.append(today)
                params.append(today)
            elif deadline == 'overdue':
                query += " AND t.deadline < DATE(?) AND t.status NOT IN ('completed', 'cancelled')"
                params.append(today)
...
def api_reject_report(report_id):
    ...
    try:
        ...
        c.execute("UPDATE reports SET status = 'rejected', admin_comment = ? WHERE id = ?", 
                 (comment, report_id))
...
def debugger():
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM users WHERE last_login IS NOT NULL AND last_login > ?", (seven_days_ago,))
...
def set_password():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(app.config['DATABASE'])
        c = conn.cursor()
        c.execute("UPDATE users SET password = ? WHERE user_id = ?", (hashed_pw, user_id))
        conn.commit()
        conn.close()
...
def download_file(filename):
    ...
    if os.path.exists(file_path):
        ...
        c.execute("SELECT assigned_to, assigned_by FROM user_tasks WHERE file_path = ?", (safe_filename,))
        task = c.fetchone()
        ...
        c.execute("SELECT user_id FROM reports WHERE file_path = ?", (safe_filename,))
        report = c.fetchone()
        
        # Agar fayl bazada topilmasa, boshqa fayllarni tekshirish
        if not report:
            c.execute("SELECT user_id FROM reports WHERE file_path LIKE ?", (f'%{safe_filename}%',))
            report = c.fetchone()
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if 'yoshlar_guruhi' in data:
                update_fields.append("yoshlar_guruhi = ?")
                update_values.append(data['yoshlar_guruhi'])
            
            if 'qomita' in data:
                update_fields.append("qomita = ?")
...
def add_points_to_user(user_id):
    ...
    try:
        ...
        # Yangilash
        cursor.execute('UPDATE users SET rating = ? WHERE user_id = ?', 
                      (new_rating, user_id))
